...
def plot_max_pain(ticker, option_date):
	...
	call_strikes = np.array(calls['strike'])
	call_oi = np.array(calls['openInterest'])
	put_strikes = np.array(puts['strike'])
	put_oi = np.array(puts['openInterest'])

	# Calculate total value of calls and puts for each strike price
	underlying_price = nq.history().iloc[-1]['Close']
	call_values = (np.maximum(call_strikes - underlying_price, 0) * call_oi)
	put_values = (np.maximum(underlying_price - put_strikes, 0) * put_oi)
	...
	fig.update_layout(
		title={
		'text':f'${ticker.upper()} Max Pain on {option_date}',
		"x": 0.5,
		"xanchor": "center",
		"yanchor": "top",
		"font":{"size":22}
